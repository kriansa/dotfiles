...
@on(NetworkConnectionStatus, connection_name="VPN@Admin")
@use(Polybar)
async def vpn_admin_status(status: dict, polybar: Polybar):
    color = colors.BAR_FG if status["connected"] else colors.BAR_DISABLED
    await polybar("vpn-adm", polybar.fmt("", color=color))
...
async def battery_warning(status: dict, notify: Notify):
    ...
    if level > 5:
        ...
        await sleep(5)
        await cmd("sudo", "systemctl", "hibernate")
...
class PowerState:  # pylint: disable=too-few-public-methods
    ...
    async def hibernate():
        """
        Before executing any hibernation action, we first check if the system just returned from
        hibernation, so that we don't re-hibernate in cases where we can't verify if the hibernation
        request has been called twice.
        """
        entry = await cmd(
            "journalctl", "--unit=hibernate.target", "--lines=1", "--no-pager",
            "--output=json", "--output-fields=__REALTIME_TIMESTAMP"
        )
        if not entry:
            return

        json_entry = json.loads(entry['stdout'])
        time = datetime.fromtimestamp(int(json_entry["__REALTIME_TIMESTAMP"]) / 1e6)
        if datetime.today() - time > timedelta(seconds=5):
            await cmd("sudo", "systemctl", "hibernate")
...
async def configure_screens(status: dict, notify: Notify):
    ...
    # Order the list by the monitor name
    screens.sort(key=lambda output: output[1])
    ...
    match screens:
        ...
        # Configured layouts
        case [[laptop_output, "LAPTOP"], [primary_output, "PRIMARY"]]:
            await cmd(
                "xrandrw", "--outputs-off",
                "--output", primary_output, "--preferred", "--pos", "0x0", "--primary",
                "--output", laptop_output, "--preferred", "--pos", "3440x740",
            )
    ...
    # Detect monitors on herbstluftwm
    await cmd("herbstclient", "detect_monitors")

    # Apply changes to polybar
    await cmd("systemctl", "reload", "--user", "polybar")

    # Apply background (random per day)
    random.seed(str(date.today()))
    await cmd(
        "feh", "--bg-fill", "--no-fehbg",
        random.choice(list(Path.home().joinpath(".local/share/backgrounds").glob("*.png")))
    )
